...
export function NodeInspector({
  ...
  if (!detail) {
    return (
      <div
        className={cn(
          "flex flex-col items-center justify-center p-6 text-center h-full select-none text-[var(--text-secondary)]",
          className,
        )}
        data-testid="node-inspector-empty"
      >
        <div className="w-10 h-10 rounded-xl bg-[var(--surface-panel-secondary)] border border-[var(--border-subtle)] flex items-center justify-center text-[var(--text-muted)] mb-3">
  ...
  return (
    ...
    >
      ...
      {!detail.isVisibleOnCanvas && (
        <div
          className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-200 space-y-2"
        ...
        >
          ...
          <div className="flex items-center gap-2 pt-1">
            <Button
              ...
              className="text-[11px] h-6 px-2 gap-1 bg-amber-500/20 hover:bg-amber-500/30 text-amber-200 border border-amber-500/40"
      ...
      <div className="border-b border-[var(--border-subtle)] pb-3">
        ...
        <div className="flex items-center gap-2 mt-2">
          ...
          <div className="min-w-0 flex-1">
            <h3
              className="text-sm font-semibold font-mono text-[var(--text-primary)] truncate"
      ...
      <div className="grid grid-cols-2 gap-2 p-2.5 rounded-xl bg-[var(--surface-canvas)] border border-[var(--border-subtle)] font-mono text-[11px]">
        ...
        <div className="space-y-0.5 pt-1 border-t border-[var(--border-subtle)]/40">
          <span className="text-[var(--text-muted)] text-[10px]">Lines</span>
          <p className="text-xs font-semibold text-[var(--text-primary)]">
            {detail.metrics.lineCount}
          </p>
        </div>
        <div className="space-y-0.5 pt-1 border-t border-[var(--border-subtle)]/40">
          <span className="text-[var(--text-muted)] text-[10px]">Symbols</span>
          <p className="text-xs font-semibold text-[var(--text-primary)]">
          ...
          <div className="space-y-1 max-h-48 overflow-y-auto pr-1">
            {detail.directoryDetails.fileList.map((f) => (
              <button
                ...
                className="w-full flex items-center justify-between p-1.5 rounded-lg bg-[var(--surface-canvas)] hover:bg-[var(--surface-panel-secondary)] border border-[var(--border-subtle)] text-left font-mono text-[11px] transition-colors cursor-pointer group"
      ...
      <div className="space-y-2">
        <div className="text-[11px] font-semibold text-[var(--text-primary)] flex items-center gap-1.5">
        ...
        {detail.incomingDependencies.length > 0 ? (
          <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
            {detail.incomingDependencies.map((dep, idx) => (
              <button
                ...
                className="w-full flex items-center justify-between gap-2 p-1.5 rounded-lg bg-[var(--surface-canvas)] hover:bg-[var(--surface-panel-secondary)] border border-[var(--border-subtle)] text-left font-mono text-[11px] transition-colors cursor-pointer group"
                title="Click to center on canvas and reveal declaration in Monaco code viewer"
                data-testid={`incoming-dep-${idx}`}
              >
                <div className="min-w-0 flex-1">
                  <div className="text-[var(--text-primary)] font-medium truncate flex items-center gap-1">
                    <span>{dep.sourceFileName}</span>
                    <ExternalLink className="w-2.5 h-2.5 text-[var(--text-muted)] group-hover:text-[var(--accent-primary)] opacity-0 group-hover:opacity-100 transition-opacity" />
                  </div>
          ...
          <p className="text-[11px] text-[var(--text-muted)] italic font-mono pl-1">
      ...
      {detail.exportedSymbols.length > 0 && (
        <div className="space-y-2">
          <div className="text-[11px] font-semibold text-[var(--text-primary)] flex items-center gap-1.5">
            <Code2 className="w-3.5 h-3.5 text-[var(--syntax-ts)]" />
          ...
          <div className="space-y-1 max-h-40 overflow-y-auto pr-1">
            {detail.exportedSymbols.map((sym) => (
              ...
              >
                <span className="text-[var(--text-secondary)] group-hover:text-[var(--text-primary)] font-medium truncate">
                  {sym.name}
                </span>
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1 max-h-40 overflow-y-auto pr-1">
            {detail.internalSymbols.map((sym) => (
              ...
              >
                <span className="text-[var(--text-muted)] group-hover:text-[var(--text-primary)] truncate">
                  {sym.name}
                </span>
                <div className="flex items-center gap-1.5 shrink-0">

...
3. Construct the workspace layout Zustand store with `partialize` persistence, hydration guards, and responsive breakpoint listeners (`src/stores/workspace-store.ts`), satisfies **AC-7**, **AC-9**
4. Install `react-resizable-panels`, `lucide-react`, and `clsx`, and build the `ResizableSplitPane` workspace layout with responsive drawer fallbacks (`src/components/layout/`), satisfies **AC-2**, **AC-3**, **AC-9**

...
The server no longer falls back to its own `GITHUB_TOKEN` for anonymous ingests, because that made the token's private repository access reachable by anyone. The tradeoff is that anonymous users share one unauthenticated GitHub quota, so the recovery dialog from spec 0009 becomes the normal path when it runs out.

Ingestion also gained request guards and resource bounds, because the archive was buffered and expanded before any cap applied and the route had no rate limit. A per request nonce was chosen over a hash for the `Content-Security-Policy` because Next.js emits its own inline bootstrap scripts that a single hash cannot cover. The cost is on demand page rendering. The shared reasoning for cookies, origin checks, and rate limits is recorded in spec 0008.
